...
SET nama = 'Pro Bulanan',
    ...
    durasi_bulan = 1,
    deskripsi = 'Paket Pro 1 bulan. Harga awal Rp10.000, renewal Rp15.000/bulan.'
WHERE kode = 'pro';
...
SELECT 'pro_6_bulan', 'Pro 6 Bulan', 50000, 75000, 6,
       'Paket Pro 6 bulan. Harga awal Rp50.000, renewal Rp75.000/6 bulan.',
       fitur_json, 'aktif'
...
INSERT INTO paket_langganan
  (kode, nama, harga_bulanan, harga_perpanjangan, durasi_bulan, deskripsi, fitur_json, status)
SELECT 'pro_1_tahun', 'Pro 1 Tahun', 100000, 150000, 12,
       'Paket Pro 1 tahun. Harga awal Rp100.000, renewal Rp150.000/tahun.',
       fitur_json, 'aktif'
FROM paket_langganan
WHERE kode = 'pro'
  AND NOT EXISTS (SELECT 1 FROM paket_langganan WHERE kode = 'pro_1_tahun');

-- Isi harga renewal untuk data paket lama jika kolom baru ditambahkan.
UPDATE paket_langganan
SET harga_perpanjangan = CASE
  WHEN durasi_bulan = 1 THEN 15000
  WHEN durasi_bulan = 6 THEN 75000
  WHEN durasi_bulan = 12 THEN 150000
  ELSE harga_bulanan
END
WHERE harga_perpanjangan IS NULL OR harga_perpanjangan = 0;
